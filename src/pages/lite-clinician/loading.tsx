...
export default function ClinicianLoading() {
  ...
  React.useEffect(() => {
    const profile = readLiteProfile(LITE_CLINICIAN);
  ...
  React.useEffect(() => {
    let cancelled = false;
    const go = () => {
      if (!cancelled) Router.replace(`${LITE_CLINICIAN.basePath}/report`);
    };
    ...
    Promise.all([fetchLiteReport(result, LITE_CLINICIAN), delay(MIN_VISIBLE_MS)])
      .then(([report]) => {
        if (cancelled) return;
