 ...
 *
 * A copy of /lite-one aimed at clinicians rather than consumers: same game,
 * same quiz, same report, its own routes and its own leads table
 * (public.liteclinician_leads) so a clinician audience never dilutes the
 * consumer funnels' conversion numbers. Individual sends are separated within
 * that table by utm_campaign, which LITE_CLINICIAN.defaultCampaign backstops
 * when a link arrives without one.
 *
 * The copy on these pages is still the consumer wording, verbatim. That is the
 * starting point, not the destination — this page, the quiz framing and the
 * report's closing sections are where clinician-facing language goes, and the
 * duplicate exists so that rewrite can happen without touching a live funnel.
 *
 ...
 * palette. hookReportPath is what actually separates the funnels after the
