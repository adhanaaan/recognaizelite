...
export default function ClinicianResults() {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    attemptIdRef.current = readOrCreateAttemptId(LITE_CLINICIAN);

    fetchLiteReport(result, LITE_CLINICIAN)
      .then((report) => {
        if (cancelled) return;
        reportRef.current = report;
        stashReport(report, LITE_CLINICIAN);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/save-lead", {
        ...
        body: JSON.stringify({
          clinic: LITE_CLINICIAN.clinic,
          attemptId: attemptIdRef.current || readOrCreateAttemptId(LITE_CLINICIAN),
          name: trimmedName,
          email: trimmedEmail,
          ageRange,
          gender,
      ...
      Router.push(`${LITE_CLINICIAN.basePath}/loading`);
    } catch (err) {
