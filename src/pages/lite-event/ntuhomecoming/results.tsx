...
export default function NtuHomecomingResults() {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    attemptIdRef.current = readOrCreateAttemptId(NTU_HOMECOMING);

    fetchLiteReport(result, NTU_HOMECOMING)
      .then((report) => {
        if (cancelled) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/save-lead", {
        ...
        body: JSON.stringify({
          clinic: NTU_HOMECOMING.clinic,
          attemptId: attemptIdRef.current || readOrCreateAttemptId(NTU_HOMECOMING),
          name: trimmedName,
          email: trimmedEmail,
          ageRange,
          gender,
      ...
      Router.push(`${NTU_HOMECOMING.basePath}/loading`);
    } catch (err) {
