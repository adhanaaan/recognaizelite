...
/**
 * /lite-event/ntuhomecoming — entry. The NTU Homecoming event link.
 *
 ...
 */
export default function NtuHomecomingEntry() {
  const { lang, setLang, enabled } = useLiteEventLang();
  const t = liteEventCopy(lang);

  useEffect(() => {
    setHookClinic(NTU_HOMECOMING.hookClinic);
    setHookEntryPath(NTU_HOMECOMING.basePath);
    setHookReportPath(`${NTU_HOMECOMING.basePath}/game-complete`);
  ...
  const start = () => Router.push(`${NTU_HOMECOMING.basePath}/ready`);
