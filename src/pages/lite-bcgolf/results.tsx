...
export default function BcGolfResults() {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    attemptIdRef.current = readOrCreateAttemptId(LITE_BCGOLF);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/save-lead", {
        ...
        body: JSON.stringify({
          clinic: LITE_BCGOLF.clinic,
          attemptId: attemptIdRef.current || readOrCreateAttemptId(LITE_BCGOLF),
          name: trimmedName,
          email: trimmedEmail,
          ageRange,
          gender,
      ...
      Router.push(`${LITE_BCGOLF.basePath}/loading`);
    } catch (err) {
