...
import { EVENT, PRODUCT } from "src/data/liteBcGolfContent";
import { LITE_BCGOLF } from "src/utils/liteOne";

/**
 * /lite-bcgolf's report — /lite-two's v2 scroll-snapped story, for the event.
 *
 * The layout, charts, motion and personalised copy are /lite-two/report's,
 * unchanged. One section differs, and it is the reason this is a separate file
 * rather than a shared component: /lite-two closes on a priced offer — normal
 * price struck through, a festival discount, a total, and a button into the
 * $20 voucher page. None of that can appear at someone else's fundraiser.
 *
 * So section 9 is replaced. Where /lite-two sells, this points at the people
 * standing in the room, reusing the wording already approved for this funnel's
 * simpler report: "Ask our team to unlock it". No price, no voucher, no
 * checkout. /lite-bcgolf/report-full still exists and still renders, but
...
export default function BcGolfReportV2() {
  const scrollerRef = React.useRef<HTMLDivElement>(null);
  const heroRef = React.useRef<HTMLDivElement>(null);
  const reduced = useReducedMotion();
  const { query } = useRouter();

  const data = useReportData(LITE_BCGOLF);
  ...
  const share = async (text: string) => {
    const url = typeof window === "undefined" ? "" : `${window.location.origin}${LITE_BCGOLF.basePath}`;
    try {
  ...
  return (
    <>
      <Head>
        <title>Your result | {PRODUCT.name}</title>
          ...
          >
            ...
            <SnapSection id="recognaize">
              <Cascade amount={0.2}>
                ...
                {/* /lite-two puts a "Take the assessment" button here, into the
                    voucher page. A button that opens a checkout is the one thing
                    this page must not do, and a button that goes nowhere reads as
                    broken — so the same space states who to talk to instead. */}
                <motion.div
                  variants={rise}
                  className="mt-8 rounded-[26px] border border-[#F2DDCE] bg-white p-6 text-center shadow-[0_18px_46px_-28px_rgba(90,40,10,0.28)]"
                >
                  <p className="font-display text-[19px] font-extrabold tracking-[-0.01em] text-[#1C110A]">
                    Ask our team to set it up
                  </p>
                  <p className="mt-2 text-[14.5px] leading-[1.6] text-[#6B5245]">
                    We are here today. Show this screen to a member of our team and
                    they will walk you through the full assessment.
                  </p>
                </motion.div>
            ...
            {/* ------------------------------------- 9 · next steps --- */}
            {/* /lite-two's section 9 is the offer: normal price struck through,
                a festival discount, a total, and a button into the $20 voucher.
                At someone else's fundraiser we are a guest, so there is no
                price on this page and nothing to buy. What replaces it is the
                honest version of the same section — what the full assessment
                is, and that the people who run it are standing in the room. */}
            <SnapSection id="next">
              <Cascade amount={0.15}>
                ...
                <motion.h2
                  ...
                  className="mt-3 font-display text-[clamp(28px,7.6vw,36px)] font-extrabold leading-[1.12] tracking-[-0.025em] text-[#1C110A]"
                >
                  Today measured one domain. The full assessment measures four.
                </motion.h2>
                <motion.p variants={rise} className="mt-4 text-[15.5px] leading-[1.6] text-[#6B5245]">
                  A 60-second game can see processing speed and nothing else. {PRODUCT.name}{" "}
                  covers memory, attention and executive function as well, and reports them
                  against the same age-matched reference this screen used.
                </motion.p>
                ...
                >
                  ...
                  <p className="mt-1 text-[13px] font-bold text-[#B79C8E]">{PRODUCT.company}</p>
                  <p className="mt-2 text-[13.5px] leading-[1.55] text-[#6B5245]">
                    Processing speed &middot; Memory &middot; Attention &middot; Executive function
                  </p>
                  ...
                  >
                    {[
                      "Four games, about 15 minutes, on a tablet.",
                      "A report across all four domains, not one.",
                      "Developed and validated in a Singapore community cohort.",
                    ].map((item) => (
                      <motion.li
                        key={item}
                        variants={rise}
                        className="flex items-start gap-2.5 text-[14.5px] leading-[1.55] text-[#6B5245]"
                      >
                        <span aria-hidden className="mt-[7px] size-1.5 shrink-0 rounded-full bg-[#C4400E]" />
                        <span>{item}</span>
                      </motion.li>
                    ))}
                  </motion.ul>
                ...
                <motion.div
                  variants={pop}
                  className="mt-7 rounded-[26px] border border-[#E6C9A8] bg-[#FFFDFB] p-6 text-center"
                >
                  <p className="font-display text-[20px] font-extrabold tracking-[-0.01em] text-[#1C110A]">
                    Ask our team to unlock it
                  </p>
                  <p className="mt-2 text-[14.5px] leading-[1.6] text-[#6B5245]">
                    Show this screen to a member of our team and they will set up the full
                    assessment for you.
                  </p>
                </motion.div>
