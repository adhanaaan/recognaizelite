 ...
 * /lite-two — entry.
 *
 * /lite-one's flow with the report replaced by the personalised v2 design:
 * same landing, game, quiz and lead form, its own routes and its own leads
 * table (public.litetwo_leads) so its numbers never blend into the /lite-one
 * baseline. The divergence is deliberate and lives entirely on the report
 * page, which splits its copy by audience (optimizers vs seniors from 40) and
 * by how the score came out.
 *
 * hookClinic is "LiteTwo", which isLiteOneMode() also matches, so the shared
 ...
 */
export default function LiteTwoEntry() {
  useEffect(() => {
    setAppLanguage("ENGLISH");
    setHookClinic(LITE_TWO.hookClinic);
    setHookEntryPath(LITE_TWO.basePath);
    setHookReportPath(`${LITE_TWO.basePath}/game-complete`);
  ...
  const start = () => Router.push(`${LITE_TWO.basePath}/ready`);
