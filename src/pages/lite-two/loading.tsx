...
export default function LiteTwoLoading() {
  const { result } = useResultStore();
  const [name, setName] = React.useState("");
  const [crumb, setCrumb] = React.useState(0);

  React.useEffect(() => {
    const profile = readLiteProfile(LITE_TWO);
  ...
  React.useEffect(() => {
    let cancelled = false;
    const go = () => {
      if (!cancelled) Router.replace(`${LITE_TWO.basePath}/report`);
    };

    const stashed = readStashedReport(LITE_TWO);
    if (stashed) {
    ...
    Promise.all([fetchLiteReport(result, LITE_TWO), delay(MIN_VISIBLE_MS)])
      .then(([report]) => {
        if (cancelled) return;
