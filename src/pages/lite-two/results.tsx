...
export default function LiteTwoResults() {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    attemptIdRef.current = readOrCreateAttemptId(LITE_TWO);

    fetchLiteReport(result, LITE_TWO)
      .then((report) => {
        if (cancelled) return;
        reportRef.current = report;
        stashReport(report, LITE_TWO);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/save-lead", {
        ...
        body: JSON.stringify({
          clinic: LITE_TWO.clinic,
          attemptId: attemptIdRef.current || readOrCreateAttemptId(LITE_TWO),
          name: trimmedName,
          email: trimmedEmail,
          ageRange,
          gender,
      ...
      Router.push(`${LITE_TWO.basePath}/loading`);
    } catch (err) {
