...
export default function Act4HealthResults() {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    attemptIdRef.current = readOrCreateAttemptId(ACT4HEALTH);

    fetchLiteReport(result, ACT4HEALTH)
      .then((report) => {
        if (cancelled) return;
        reportRef.current = report;
        stashReport(report, ACT4HEALTH);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/save-lead", {
        ...
        body: JSON.stringify({
          clinic: ACT4HEALTH.clinic,
          attemptId: attemptIdRef.current || readOrCreateAttemptId(ACT4HEALTH),
          name: trimmedName,
          email: trimmedEmail,
          ageRange,
          gender,
      ...
      Router.push(`${ACT4HEALTH.basePath}/loading`);
    } catch (err) {
  ...
  return (
    <>
      <Head>
        <title>Where should we send your results? | Act4Health</title>
      </Head>

      <LiteShell scroll className="px-5 pb-12 sm:px-8" partnerLogo={ACT4HEALTH_LOGO}>
