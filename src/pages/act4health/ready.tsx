    ...
    <span className="relative block w-[94px]" aria-hidden>
      ...
      <svg viewBox="0 0 96 52" className="mt-3 block w-full">
        <defs>
           ...
           * rendered alongside the other lite ready screens, but the id is
           * namespaced anyway so the copies can't collide if that ever changes.
           */}
          <linearGradient id="a4h-step-curve" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#f77528" stopOpacity="0.45" />
            <stop offset="100%" stopColor="#f77528" stopOpacity="0.04" />
          </linearGradient>
        </defs>
        <path d="M2 40C16 40 18 10 46 10s30 30 48 30H2Z" fill="url(#a4h-step-curve)" />
        <path
...
export default function Act4HealthReady() {
  return (
    <>
      <Head>
        <title>What happens next | Act4Health</title>
      </Head>

      <LiteShell partnerLogo={ACT4HEALTH_LOGO}>
        <div className="relative flex flex-1 flex-col justify-center overflow-y-auto px-6 py-6">
          <div className="mx-auto w-full max-w-[400px]">
            ...
            >
              <LiteButton onClick={() => Router.push(`${ACT4HEALTH.basePath}/challenge`)}>
                I&apos;m ready!
              </LiteButton>
            </div>
          </div>
