...
export default function Act4HealthLoading() {
  const { result } = useResultStore();
  const [name, setName] = React.useState("");
  const [crumb, setCrumb] = React.useState(0);

  React.useEffect(() => {
    const profile = readLiteProfile(ACT4HEALTH);
  ...
  React.useEffect(() => {
    let cancelled = false;
    const go = () => {
      if (!cancelled) Router.replace(`${ACT4HEALTH.basePath}/report`);
    };

    const stashed = readStashedReport(ACT4HEALTH);
    if (stashed) {
    ...
    Promise.all([fetchLiteReport(result, ACT4HEALTH), delay(MIN_VISIBLE_MS)])
      .then(([report]) => {
        if (cancelled) return;
  ...
  return (
    <>
      <Head>
        <title>Building your profile | Act4Health</title>
      </Head>
