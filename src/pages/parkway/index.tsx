 ...
 */
export default function ParkwayEntry() {
  const { lang, setLang, enabled } = useLiteEventLang();
  const t = liteEventCopy(lang);

  useEffect(() => {
    setHookClinic(PARKWAY.hookClinic);
    setHookEntryPath(PARKWAY.basePath);
    setHookReportPath(`${PARKWAY.basePath}/game-complete`);
  ...
  const start = () => Router.push(`${PARKWAY.basePath}/ready`);
