...
export default function ParkwayLoading() {
  ...
  React.useEffect(() => {
    const profile = readLiteProfile(PARKWAY);
  ...
  React.useEffect(() => {
    let cancelled = false;
    const go = () => {
      if (!cancelled) Router.replace(`${PARKWAY.basePath}/report`);
    };

    const stashed = readStashedReport(PARKWAY);
    if (stashed) {
    ...
    Promise.all([fetchLiteReport(result, PARKWAY), delay(MIN_VISIBLE_MS)])
      .then(([report]) => {
        if (cancelled) return;
