...
/**
 * /lite-event-template — entry. The template copy of the corporate-event funnel.
 *
 ...
 */
export default function LiteEventTemplateEntry() {
  const { lang, setLang, enabled } = useLiteEventLang();
  const t = liteEventCopy(lang);

  useEffect(() => {
    setHookClinic(LITE_EVENT_TEMPLATE.hookClinic);
    setHookEntryPath(LITE_EVENT_TEMPLATE.basePath);
    setHookReportPath(`${LITE_EVENT_TEMPLATE.basePath}/game-complete`);
  ...
  const start = () => Router.push(`${LITE_EVENT_TEMPLATE.basePath}/ready`);
