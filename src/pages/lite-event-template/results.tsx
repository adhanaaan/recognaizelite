...
export default function LiteEventTemplateResults() {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    attemptIdRef.current = readOrCreateAttemptId(LITE_EVENT_TEMPLATE);

    fetchLiteReport(result, LITE_EVENT_TEMPLATE)
      .then((report) => {
        if (cancelled) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/save-lead", {
        ...
        body: JSON.stringify({
          clinic: LITE_EVENT_TEMPLATE.clinic,
          attemptId: attemptIdRef.current || readOrCreateAttemptId(LITE_EVENT_TEMPLATE),
          name: trimmedName,
          email: trimmedEmail,
          ageRange,
          gender,
      ...
      Router.push(`${LITE_EVENT_TEMPLATE.basePath}/loading`);
    } catch (err) {
