...
/**
 * /clinic-signup's report — the v2 scroll-snapped story, personalised.
 *
...
export default function ClinicSignupReport() {
  ...
  const data = useReportData(CLINIC_SIGNUP);
  ...
  React.useEffect(() => {
    const stored = readReportInterest(CLINIC_SIGNUP);
    if (stored) {
  ...
  const markInterested = () => {
    ...
    setInterested(true);
    stashReportInterest({ interested: true }, CLINIC_SIGNUP);
    void recordReportInterest({ interested: true }, CLINIC_SIGNUP, { lang });
  };
  const toggleTips = (next: boolean) => {
    setTipsOptIn(next);
    stashReportInterest({ tipsOptIn: next }, CLINIC_SIGNUP);
    void recordReportInterest({ tipsOptIn: next }, CLINIC_SIGNUP, { lang });
  };
  const share = async (text: string) => {
    const url = typeof window === "undefined" ? "" : `${window.location.origin}${CLINIC_SIGNUP.basePath}`;
    try {
