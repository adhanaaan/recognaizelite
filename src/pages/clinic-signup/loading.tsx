 ...
 *
 * The flow is /lite-event-template's, less one screen: this funnel takes the
 * name, email and consent on its landing page, so there is no lead form after
 * the quiz and the quiz hands straight here. See CLINIC_SIGNUP in
...
export default function ClinicSignupLoading() {
  ...
  React.useEffect(() => {
    const profile = readLiteProfile(CLINIC_SIGNUP);
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    const go = () => {
      if (!cancelled) Router.replace(`${CLINIC_SIGNUP.basePath}/report`);
    };
      ...
      try {
        await fetch("/api/save-lead", {
          ...
          body: JSON.stringify({
            ...
            attemptId: readOrCreateAttemptId(CLINIC_SIGNUP),
            name: profile.name,
            email: profile.email,
            ageRange,
            gender,
            ...
            // Re-sent rather than left to the row: this is an update, and the
            // consent is the thing the whole run was conditional on.
     ...
     * failed.
     */
    const stashed = readStashedReport(CLINIC_SIGNUP);
