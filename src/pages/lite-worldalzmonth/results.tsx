...
export default function WorldAlzResults() {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    attemptIdRef.current = readOrCreateAttemptId(LITE_WORLDALZ);

    fetchLiteReport(result, LITE_WORLDALZ)
      .then((report) => {
        if (cancelled) return;
        reportRef.current = report;
        stashReport(report, LITE_WORLDALZ);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/save-lead", {
        ...
        body: JSON.stringify({
          clinic: LITE_WORLDALZ.clinic,
          attemptId: attemptIdRef.current || readOrCreateAttemptId(LITE_WORLDALZ),
          name: trimmedName,
          email: trimmedEmail,
          ageRange,
          gender,
      ...
      Router.push(`${LITE_WORLDALZ.basePath}/loading`);
    } catch (err) {
