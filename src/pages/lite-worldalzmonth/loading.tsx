...
export default function WorldAlzLoading() {
  const { result } = useResultStore();
  const [name, setName] = React.useState("");
  const [crumb, setCrumb] = React.useState(0);

  React.useEffect(() => {
    const profile = readLiteProfile(LITE_WORLDALZ);
  ...
  React.useEffect(() => {
    let cancelled = false;
    const go = () => {
      if (!cancelled) Router.replace(`${LITE_WORLDALZ.basePath}/report`);
    };
    ...
    Promise.all([fetchLiteReport(result, LITE_WORLDALZ), delay(MIN_VISIBLE_MS)])
      .then(([report]) => {
        if (cancelled) return;
