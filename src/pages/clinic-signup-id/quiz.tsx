...
import { QUESTIONS_ID_BY_ID } from "src/data/brainHealthQuestions.id";
import { STAT_CARDS_BY_ID, type StatCard } from "src/data/brainHealthStatCards";
import { STAT_CARDS_ID_BY_ID } from "src/data/brainHealthStatCards.id";
import {
  useClinicSignupIdLang,
  type ClinicSignupIdLang,
} from "src/i18n/clinicSignupId";
import { clinicSignupIdCopy } from "src/i18n/clinicSignupIdCopy";
import type { LiteEventCopy } from "src/i18n/liteEventCopy";
...
import { CLINIC_SIGNUP_ID, stashQuizResult } from "src/utils/liteOne";
...
/**
 * The two question banks, keyed by the funnel's language. Both carry the same
 * question ids, option ids and scores — only the wording differs — so
 * `computeScore` (which reads the English bank) grades an Indonesian run
 * exactly as it grades an English one, and the answers written to
 * liteevent_leads stay comparable across languages and across funnels.
 *
 * English is taken from the template's bank rather than the shared one, so a
 * wording change made for /lite-event-template reaches this funnel's English
 * too; the Indonesian bank is this family's own. See
 * src/data/liteEventTemplateQuestions.ts and
 * src/data/brainHealthQuestions.id.ts.
 */
const QUESTION_BANKS: Record<ClinicSignupIdLang, Record<string, Question>> = {
  en: LITE_EVENT_TEMPLATE_QUESTION_BANKS.en,
  id: QUESTIONS_ID_BY_ID,
};

const STAT_CARD_BANKS: Record<ClinicSignupIdLang, Record<string, StatCard>> = {
  en: STAT_CARDS_BY_ID,
  id: STAT_CARDS_ID_BY_ID,
};
...
export default function ClinicSignupIdQuizPage() {
  ...
  useEffect(() => {
    if (currentStep >= steps.length && steps.length > 0) {
      ...
      stashQuizResult(score, CLINIC_SIGNUP_ID);
      ...
      Router.replace(`${CLINIC_SIGNUP_ID.basePath}/loading`);
    }
