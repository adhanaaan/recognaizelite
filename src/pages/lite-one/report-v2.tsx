...
import { OFFER, TESTIMONIALS } from "src/data/liteOneContent";

/**
 * Report redesign v2, parked outside the funnel.
 *
 * Nothing links here: /lite-one/report is still the live page and
 * /lite-one/report-lab is the previous parked redesign. This version turns
 * the same narrative into a full-screen, scroll-snapped story — rank, what
 * it means, one tip, the risk result, the missing baseline, why now, the
 * full test, the offer, the quiet exit — with the sections animating in as
 * they arrive, in the mould of apple.com product pages.
 *
 * globals.css pins html/body/#__next to 100dvh with overflow hidden for the
 * game screens. Instead of unlocking those (report-lab's approach), this
 * page scrolls inside its own 100dvh container, which is also what the
 * scroll-snapping and every scroll-linked animation are anchored to. Nothing
 * global is touched, so the game screens and both other report pages are
 * unaffected.
 *
 * With no finished game on the device it renders a sample result plus a
 * preview banner, so the layout can be reviewed on its own.
 */
...
export default function ReportV2() {
  ...
  const { percentile, peers, topBand, strong, isSample, name, riskBand, riskFill } = data;
  ...
  return (
    <>
      <Head>
        ...
        {/* Lora is only used on this parked page, so it loads here rather
        ...
        <ScrollerContext.Provider value={scrollerRef}>
          ...
          >
            ...
            <div ref={heroRef}>
              ...
              >
                <Cascade amount={0.2}>
                  ...
                  <EyebrowV2>Reaction time challenge</EyebrowV2>
                  ...
                  >
                    <div className="flex items-baseline justify-between">
                      ...
                      <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#B79C8E]">
                        {data.ageRange ? `Aged ${data.ageRange}` : "All ages"}
                      </p>
                    </div>
                    <div className="mt-4">
                    ...
                    <p className="mt-4 text-[15px] leading-[1.55] text-[#6B5245]">
                      {strong
                        ...
                        : `That is the middle of your age band on ${domain} — and it moves faster than people expect.`}
                    </p>
                  </motion.div>
                </Cascade>
                ...
                >
                  <Serif className="text-[20px] text-[#B4653C]">Tell me more</Serif>
                  <motion.svg
                    aria-hidden
                    viewBox="0 0 24 24"
            ...
            >
              <Cascade>
                ...
                <motion.p variants={rise} className="mt-6 text-[17px] leading-[1.6] text-white/75">
                  With high processing speed, you can:
                </motion.p>
                <div className="mt-5 space-y-3">
                  {SPEED_PERKS.map((perk, i) => (
                    <motion.div
                      key={perk}
                      variants={rise}
                ...
                >
                  {strong
                    ? "If those feel effortless, your score just told you why."
                    : "If any of those feel like work lately, your score is the reason."}
                </motion.p>
              </Cascade>
            </SnapSection>
            ...
            <SnapSection id="risk">
              <Cascade amount={0.15}>
                ...
                <motion.p variants={rise} className="mt-4 text-[16px] leading-[1.6] text-[#41586B]">
                  We looked at your risk factors too — health and lifestyle habits like high blood
                  pressure, poor sleep, or not enough exercise can slow your brain down over time.
                </motion.p>
            ...
            <SnapSection id="baseline">
              <Cascade amount={0.2}>
                <EyebrowV2>Your baseline so far</EyebrowV2>
                <motion.h2
                  variants={rise}
                ...
                <motion.div variants={rise} className="mt-6 space-y-3 text-[15px] leading-[1.6] text-[#6B5245]">
                  <p>The speed game and your quiz answers gave us two axes — speed and risk.</p>
                  <p>
                    But your brain doesn&apos;t work on two dimensions. Memory, attention, and
                    executive function each tell a different story, and you can score well on one
                    while struggling with another.
                  </p>
                </motion.div>
              </Cascade>
            </SnapSection>
            ...
            <SnapSection id="recognaize">
              <Cascade amount={0.2}>
                <EyebrowV2>Take the test</EyebrowV2>
                <motion.h2
                  variants={rise}
                  className="mt-3 font-display text-[clamp(28px,7.6vw,36px)] font-extrabold leading-[1.12] tracking-[-0.025em] text-[#1C110A]"
                >
                  ReCOGnAIze, a digital brain performance assessment
                </motion.h2>
                <motion.p variants={rise} className="mt-4 text-[15.5px] leading-[1.6] text-[#6B5245]">
                  Validated against MRI scans, built on a five-year NTU study of 1,500 people and
                  published in <Serif>Alzheimer&apos;s &amp; Dementia</Serif>.
                </motion.p>
                ...
                <motion.div variants={stagger} className="mt-6 grid gap-3 sm:grid-cols-3">
                  {TESTIMONIALS.map(({ name: who, age, quote }, i) => (
                    <motion.figure
                      key={who}
                      variants={rise}
            ...
            >
              <Cascade amount={0.2}>
                ...
                >
                  The best time to set a baseline is while you are <Serif>sharp</Serif>.
                </motion.h2>
            ...
            <SnapSection id="sample">
              <Cascade amount={0.2}>
                ...
                <motion.p variants={rise} className="mt-4 text-[15.5px] leading-[1.6] text-[#6B5245]">
                  The full assessment measures memory, attention and executive function. Here&apos;s
                  a sample of the report you will get for every domain.
                </motion.p>
            ...
            <SnapSection id="closing">
              <Cascade amount={0.2}>
                <motion.figure variants={rise}>
                  ...
                  <blockquote className="-mt-5 text-[21px] leading-[1.55] tracking-[-0.01em] text-[#1C110A]">
                    Tomorrow&apos;s meeting is a tough one, and if I don&apos;t have my attention
                    covered, I can&apos;t perform well. Thanks to checking my brain, I&apos;m aware
                    of how to optimize it now. Glad I found <Serif>ReCOGnAIze</Serif>!
                  </blockquote>
                  <figcaption className="mt-4 text-[14.5px] text-[#8A6A58]">
                    — Chelsea, {data.ageRange ? `aged ${data.ageRange} like you` : "similar age as you"}
                  </figcaption>
                </motion.figure>
                ...
                >
                  ...
                  <p className="mt-3 text-[14.5px] leading-[1.6] text-[#6B5245]">
                    Your score is already in your inbox, along with a short set of strategies for
                    pushing it up.
                  </p>
                  <p className="mt-2 text-[14.5px] leading-[1.6] text-[#6B5245]">
                    When you&apos;re ready to test the remaining three brain domains, you know where
                    we are.
                  </p>
                  <button
                    type="button"
                    onClick={() => Router.replace("/lite-one")}
